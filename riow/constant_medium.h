...
public:
    constant_medium(shared_ptr<hittable> b, double d, shared_ptr<texture> a)
        : hittable(b->name + "_medium"), boundary(b), neg_inv_density(-1 / d), phase_function(make_shared<isotropic>(a)) {}
    constant_medium(shared_ptr<hittable> b, double d, color c)
        : hittable(b->name + "_medium"), boundary(b), neg_inv_density(-1 / d), phase_function(make_shared<isotropic>(c)) {}
